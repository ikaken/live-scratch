...
- [ ] `npm run tauri:dev` の実行 (❌ 再起動後も `link.exe` 未認識)
...
**次のステップ:**
1. **Visual Studio Installer** を開き、インストール済みの「Build Tools 2022」の「変更」ボタンをクリック。
2. **「C++ によるデスクトップ開発」** にチェックが入っているか確認する（入っていない場合はチェックを入れてインストール）。
3. インストール済みであれば、環境変数の設定を手動で確認するか、開発者用コマンドプロンプトの利用を検討する。
